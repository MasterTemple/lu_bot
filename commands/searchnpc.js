module.exports = {
    ...
    execute(message, args) {
        ...
        //
        const arg = message.content.slice(10).trim().split(", "); //each space is a new argument
        console.log(arg)
        var id
        var search = arg[0]
        ...
        try {
            ...
            for (var j = 0; j < (Object.keys(item["Sheet1"]).length);j++) {
                ...
                try{
                    if ((item["Sheet1"][j].displayName.toLowerCase().includes(search) || item["Sheet1"][j].name.toLowerCase().includes(search)) && (item["Sheet1"][j].displayName.toLowerCase().includes(searchExtra) || item["Sheet1"][j].name.toLowerCase().includes(searchExtra)) && (item["Sheet1"][j].type == "NPC" || item["Sheet1"][j].type == "UserGeneratedNPCs")) {
